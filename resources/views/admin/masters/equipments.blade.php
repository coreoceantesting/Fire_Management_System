<x-admin.layout>
    <x-slot name="title">Equipments</x-slot>
    <x-slot name="heading">Equipments (उपकरणे)</x-slot>
                        ...
                        <div class="card-body">
                            <div class="mb-3 row">
                                <div class="col-md-4">
                                    ...
                                    <input class="form-control" id="equipment_name" name="equipment_name" type="text" placeholder="Enter Equipment Name">
                                    <span class="text-danger error-text equipment_name_err"></span>
                                </div>
                                <div class="col-md-4">
                                    <label class="col-form-label" for="initial">Equipment Initial(उपकरणे प्रारंभिक) <span class="text-danger">*</span></label>
                    ...
                    <section class="card">
                        ...
                        <div class="card-body py-2">
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-md-4">
                                    <label class="col-form-label" for="initial">Equipment Initial(उपकरणे प्रारंभिक)<span class="text-danger">*</span></label>
                                    <input class="form-control" id="initial" name="initial" type="text" placeholder="Enter Equipment Initial">
                                    <span class="text-danger error-text initial_err"></span>
                                </div>
                            </div>
            ...
            success: function(data)
            {
                ...
                if (!data.error2)
                    swal("Successful!", data.success, "success")
                        .then((action) => {
                            window.location.href = '{{ route('equipments.index') }}';
                        });
                else
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('equipments.edit', ":model_id") }}";
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (!data.error)
                {
                    $("#editForm input[name='edit_model_id']").val(data.equipment.equipment_id);
                    $("#editForm input[name='equipment_name']").val(data.equipment.equipment_name);
                    $("#editForm input[name='initial']").val(data.equipment.initial);
                }
                else
                {
                    alert(data.error);
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('equipments.update', ":model_id") }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
...
<script>
    $("#buttons-datatables").on("click", ".rem-element", function(e) {
        ...
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('equipments.destroy', ":model_id") }}";
...
<script>
    $("#buttons-datatables").on("click", ".active-element", function(e) {
        e.preventDefault();
        swal({
            title: "Are you sure to active this equipment?",
            // text: "Make sure if you have filled Vendor details before proceeding further",
            icon: "info",
            buttons: ["Cancel", "Confirm"]
        })
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('equipments.active', ":model_id") }}";
...
<script>
    $("#buttons-datatables").on("click", ".inactive-element", function(e) {
        e.preventDefault();
        swal({
            title: "Are you sure to inactive this equipment?",
            // text: "Make sure if you have filled Vendor details before proceeding further",
            icon: "info",
            buttons: ["Cancel", "Confirm"]
        })
        .then((justTransfer) =>
        {
            ...
            {
                var model_id = $(this).attr("data-id");
                var url = "{{ route('equipments.inactive', ":model_id") }}";
